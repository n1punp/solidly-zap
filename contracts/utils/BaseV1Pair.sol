...
contract BaseV1Pair {
  ...
  constructor() {
    ...
    if (_stable) {
      name = string(
        abi.encodePacked('StableV1 AMM - ', erc20(_token0).symbol(), '/', erc20(_token1).symbol())
      );
      symbol = string(
        abi.encodePacked('sAMM-', erc20(_token0).symbol(), '/', erc20(_token1).symbol())
      );
    } else {
      name = string(
        abi.encodePacked('VolatileV1 AMM - ', erc20(_token0).symbol(), '/', erc20(_token1).symbol())
      );
      symbol = string(
        abi.encodePacked('vAMM-', erc20(_token0).symbol(), '/', erc20(_token1).symbol())
      );
    }
  ...
  function _update1(uint amount) internal {
    ...
    uint _ratio = (amount * 1e18) / totalSupply;
    if (_ratio > 0) {
      index1 += _ratio;
    }
