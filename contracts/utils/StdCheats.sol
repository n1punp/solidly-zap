...
abstract contract stdCheats {
  ...
  Vm constant vm_std_cheats = Vm(address(uint160(uint(keccak256('hevm cheat code')))));
...
library stdStorage {
  ...
  Vm constant stdstore_vm = Vm(address(uint160(uint(keccak256('hevm cheat code')))));
  ...
  function find(StdStorage storage self) internal returns (uint) {
    ...
    if (reads.length == 1) {
      ...
      emit SlotFound(who, fsig, keccak256(abi.encodePacked(ins, field_depth)), uint(reads[0]));
      self.slots[who][fsig][keccak256(abi.encodePacked(ins, field_depth))] = uint(reads[0]);
      ...
      for (uint i = 0; i < reads.length; i++) {
        ...
        if (fdat == bytes32(hex'1337')) {
          ...
          emit SlotFound(who, fsig, keccak256(abi.encodePacked(ins, field_depth)), uint(reads[i]));
          self.slots[who][fsig][keccak256(abi.encodePacked(ins, field_depth))] = uint(reads[i]);
          self.finds[who][fsig][keccak256(abi.encodePacked(ins, field_depth))] = true;
  ...
  function with_key(StdStorage storage self, uint amt) internal returns (StdStorage storage) {
  ...
  function depth(StdStorage storage self, uint _depth) internal returns (StdStorage storage) {
    self._depth = _depth;
    return self;
  }
